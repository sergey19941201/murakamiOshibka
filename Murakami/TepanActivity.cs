...
namespace Murakami
{
    [Activity(Label = "TepanActivity")]
    public class TepanActivity : Activity
    {
        protected override void OnCreate(Bundle bundle)
        {
            base.OnCreate(bundle);
            SetContentView(Resource.Layout.Tepan);

            var path = System.Environment.GetFolderPath(System.Environment.SpecialFolder.MyDocuments);
            var filename = System.IO.Path.Combine(path, "cache18.txt");
            JsonValue readJson;
        ...
        private async void ParseAndDisplay(JsonValue readJson)
        {
            ...
            add.Click += delegate
            {
                var intent = new Intent(this, typeof(ovoshchiTepanSTeliatinoy));
        ...
        private async void ParseAndDisplay1(JsonValue readJson)
        {
            ...
            add1.Click += delegate
            {
                var intent = new Intent(this, typeof(ovoshchiTepanSMoreproduktami));
                StartActivity(intent);
            };
            // Zagruska detalnogo prosmotra, ego poka net
            /* add.Click += delegate
        ...
        private async void ParseAndDisplay2(JsonValue readJson)
        {
            ...
            add2.Click += delegate
            {
                var intent = new Intent(this, typeof(tepanOvoshchi));
                StartActivity(intent);
            };
            // Zagruska detalnogo prosmotra, ego poka net
            /* add.Click += delegate
        ...
        private async void ParseAndDisplay3(JsonValue readJson)
        {
            ...
            add3.Click += delegate
            {
                var intent = new Intent(this, typeof(ovoshchiTepanSKuritsey));
                StartActivity(intent);
            };
            // Zagruska detalnogo prosmotra, ego poka net
            /* add.Click += delegate
